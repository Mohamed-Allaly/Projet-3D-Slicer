...
class DICOM(ScriptedLoadableModule):
  ...
  def __init__(self, parent):
    ...
    self.parent.contributors = [_("Steve Pieper (Isomics)"), _("Andras Lasso (PerkLab)")]
...
"""
    self.parent.icon = qt.QIcon(_(':Icons/Medium/SlicerLoadDICOM.png'))
  ...
  def setup(self):
    ...
    slicer.app.connect(_("startupCompleted()"), self.performPostModuleDiscoveryTasks)
    slicer.app.connect(_("urlReceived(QString)"), self.onURLReceived)
    ...
    self.viewFactory.setTagName_(("dicombrowser"))
  ...
  def performPostModuleDiscoveryTasks(self):
    ...
    if slicer.mrmlScene.GetTagByClassName( _("vtkMRMLScriptedModuleNode") ) != _('ScriptedModule'):
      slicer.mrmlScene.RegisterNodeClass(vtkMRMLScriptedModuleNode())

    self.initializeDICOMDatabase()

    settings = qt.QSettings()
    if settings.contains(_('DICOM/RunListenerAtStart')) and not slicer.app.commandOptions().testingEnabled:
      if settings.value(_('DICOM/RunListenerAtStart')) == _('true'):
        self.startListener()
    ...
    if not slicer.app.commandOptions().noMainWindow:
      ...
      slicer.app.settingsDialog().addPanel(_("DICOM"), self.settingsPanel)

      layoutManager = slicer.app.layoutManager()
      layoutManager.layoutChanged.connect(self.onLayoutChanged)
      layout = _("<layout type=\")horizontal\">" +"\n"
      + _(" <item>") +"\n"
      + _("  <dicombrowser></dicombrowser>") +"\n"
      +_(" </item>") +"\n"
      + _("</layout>")
  ...
  def onURLReceived(self, urlString):
    ...
    if (url.authority().lower() != _("viewer")):
      logging.debug(_("DICOM module ignores non-viewer URL: ")+urlString)
      return
    ...
    if not _("dicomweb_endpoint") in queryMap:
      logging.debug(_("DICOM module ignores URL without dicomweb_endpoint query parameter: ")+urlString)
      return
    if not _("studyUID") in queryMap:
      logging.debug(_("DICOM module ignores URL without studyUID query parameter: ")+urlString)
      return

    logging.info(_("DICOM module received URL: ")+urlString)

    accessToken = None
    if _("access_token") in queryMap:
      accessToken = queryMap[_("access_token")]

    slicer.util.selectModule_(("DICOM"))
    slicer.app.processEvents()
    from DICOMLib import DICOMUtils
    importedSeriesInstanceUIDs = DICOMUtils.importFromDICOMWeb(
      dicomWebEndpoint=queryMap[_("dicomweb_endpoint")],
      studyInstanceUID=queryMap[_("studyUID")],
  ...
  def initializeDICOMDatabase(self):
    ...
    if slicer.app.commandOptions().testingEnabled:
      ...
      slicer.dicomDatabaseDirectorySettingsKey = _('DatabaseDirectoryTest_')+ctk.ctkDICOMDatabase().schemaVersion()
      databaseDirectory = os.path.join(slicer.app.temporaryPath,
        _('temp')+slicer.app.applicationName+_('DICOMDatabase_')+ctk.ctkDICOMDatabase().schemaVersion())
    else:
      # For production
      slicer.dicomDatabaseDirectorySettingsKey = _('DatabaseDirectory_')+ctk.ctkDICOMDatabase().schemaVersion()
      settings = qt.QSettings()
      databaseDirectory = settings.value(slicer.dicomDatabaseDirectorySettingsKey)
      if not databaseDirectory:
        documentsLocation = qt.QStandardPaths.DocumentsLocation
        documents = qt.QStandardPaths.writableLocation(documentsLocation)
        databaseDirectory = os.path.join(documents, slicer.app.applicationName+_("DICOMDatabase"))
    ...
    databaseFileName = databaseDirectory + _("/ctkDICOM.sql")
  ...
  def startListener(self):
    ...
    if not slicer.dicomDatabase.isOpen:
      logging.error_(("Failed to start DICOM listener. DICOM database is not open."))
      return False

    if not hasattr(slicer, _('dicomListener')):
      dicomListener = DICOMLib.DICOMListener(slicer.dicomDatabase)

    try:
      dicomListener.start()
    except (UserWarning, OSError) as message:
      logging.error(_('Problem trying to start DICOM listener:\n %s') % message)
      return False
    if not dicomListener.process:
      logging.error_(("Failed to start DICOM listener. Process start failed."))
      return False
    slicer.dicomListener = dicomListener
    logging.info(_("DICOM C-Store SCP service started at port ")+str(slicer.dicomListener.port))

  def stopListener(self):
    if hasattr(slicer, _('dicomListener')):
      logging.info_(("DICOM C-Store SCP service stopping"))
  ...
  def addMenu(self):
    ...
    a.setText(a.tr_(("Add DICOM Data")))
    fileMenu = slicer.util.lookupTopLevelWidget(_('FileMenu'))
    if fileMenu:
      for child in fileMenu.children():
        if child.objectName == _("RecentlyLoadedMenu"):
  ...
  def setBrowserWidgetInDICOMLayout(self, browserWidget):
    ""_("Set DICOM browser widget in the custom view layout")""
    ...
    if self.viewWidget is None:
      ...
      label = qt.QLabel_(("DICOM database"))
  ...
  def onLayoutChanged(self, viewArrangement):
    ...
    dataProbe = mw.findChild_(("QWidget", "DataProbeCollapsibleWidget")) if mw else None
  ...
  def onBrowserWidgetClosed(self):
    ...
    if (layoutId == slicer.vtkMRMLLayoutNode.SlicerLayoutNone
      ...
      layoutId = qt.QSettings().value(_("MainWindow/layout"), slicer.vtkMRMLLayoutNode.SlicerLayoutInitialView)
...
class _ui_DICOMSettingsPanel:
  def __init__(self, parent):
    ...
    parent.registerProperty(slicer.dicomDatabaseDirectorySettingsKey, directoryButton,
                      _("directory"), str(qt.SIGNAL_(("directoryChanged(QString)"))),
    ...
    loadReferencesComboBox.toolTip = _("Determines whether referenced DICOM series are ") \
      _("offered when loading DICOM, or the automatic behavior if interaction is disabled. ") \
      _("Interactive selection of referenced series is the default selection")
    ...
    parent.registerProperty(
      _("DICOM/automaticallyLoadReferences"), loadReferencesComboBox,
      _("currentUserDataAsString"), str(qt.SIGNAL_(("currentIndexChanged(int)"))))

    detailedLoggingCheckBox = qt.QCheckBox()
    detailedLoggingCheckBox.toolTip = (_("Log more details during DICOM operations.")
      _(" Useful for investigating DICOM loading issues but may impact performance."))
    genericGroupBoxFormLayout.addRow(_("Detailed logging:"), detailedLoggingCheckBox)
    detailedLoggingMapper = ctk.ctkBooleanMapper(detailedLoggingCheckBox, _("checked"), str(qt.SIGNAL_(("toggled(bool)"))))
    parent.registerProperty(
      _("DICOM/detailedLogging"), detailedLoggingMapper,
      _("valueAsInt"), str(qt.SIGNAL_(("valueAsIntChanged(int)"))))
    ...
    for pluginName in plugins.keys():
      if hasattr(plugins[pluginName], _('settingsPanelEntry')):
...
class DICOMFileDialog:
  ...
  def execDialog(self):
    ""_("Not used")""
    logging.debug(_('execDialog called on %s') % self)
  ...
  def pathsFromMimeData(mimeData):
    directoriesToAdd = []
    filesToAdd = []
    if mimeData.hasFormat(_('text/uri-list')):
      urls = mimeData.urls()
      for url in urls:
  ...
  def createDefaultDatabase():
    ...
    if slicer.modules.DICOMInstance.browserWidget is None:
      slicer.util.selectModule(_('DICOM'))
    ...
    slicer.util.warningDisplay(_("Could not create a DICOM database with default settings. Please create a new database or")
      _(" update the existing incompatible database using options shown in DICOM browser."))
    return False
  ...
  def dropEvent(self):
    ...
    if not DICOMFileDialog.validDirectories(self.directoriesToAdd):
      if not slicer.util.confirmYesNoDisplay(_("Import of files that have special (non-ASCII) characters in their names is not supported.")
          _(" It is recommended to move files into a different folder and retry. Try to import from current location anyway?")):
        self.directoriesToAdd = []
        return
...
class DICOMLoadingByDragAndDropEventFilter(qt.QWidget):
  ...
  def dropEvent(self, event):
    ...
    if not DICOMFileDialog.validDirectories(self.directoriesToAdd) or not DICOMFileDialog.validDirectories(self.filesToAdd):
      if not slicer.util.confirmYesNoDisplay(_("Import from folders with special (non-ASCII) characters in the name is not supported.")
          _(" It is recommended to move files into a different folder and retry. Try to import from current location anyway?")):
        self.directoriesToAdd = []
        return
...
class DICOMWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    if hasattr(self, _("reloadCollapsibleButton")):
    ...
    uiWidget = slicer.util.loadUI(self.resourcePath(_('UI/DICOM.ui')))
    self.layout.addWidget(uiWidget)
    self.ui = slicer.util.childWidgetVariables(uiWidget)

    self.browserWidget = DICOMLib.SlicerDICOMBrowser()
    self.browserWidget.objectName = _('SlicerDICOMBrowser')
    ...
    self.ui.showBrowserButton.connect(_('clicked()'), self.toggleBrowserWidget)

    self.ui.importButton.connect(_('clicked()'), self.importFolder)
    ...
    self.ui.queryServerButton.connect(_('clicked()'), self.browserWidget.dicomBrowser, _("openQueryDialog()"))

    self.ui.toggleListener.connect(_('toggled(bool)'), self.onToggleListener)

    settings = qt.QSettings()
    self.ui.runListenerAtStart.checked = settingsValue(_('DICOM/RunListenerAtStart'), False, converter=toBool)
    self.ui.runListenerAtStart.connect(_('toggled(bool)'), self.onRunListenerAtStart)

    # Testing server - not exposed (used for development)

    self.toggleServer = qt.QPushButton_(("Start Testing Server"))
    self.ui.networkingFrame.layout().addWidget(self.toggleServer)
    self.toggleServer.connect(_('clicked()'), self.onToggleServer)

    self.verboseServer = qt.QCheckBox_(("Verbose"))
    ...
    self.ui.browserAutoHideCheckBox.checked = not settingsValue(_('DICOM/BrowserPersistent'), False, converter=toBool)
    self.ui.browserAutoHideCheckBox.stateChanged.connect(self.onBrowserAutoHideStateChanged)

    self.ui.repairDatabaseButton.connect(_('clicked()'), self.browserWidget.dicomBrowser, _("onRepairAction()"))
    self.ui.clearDatabaseButton.connect(_('clicked()'), self.onClearDatabase)

    # connect to the main window's dicom button
    mw = slicer.util.mainWindow()
    if mw:
      try:
        action = slicer.util.findChildren(mw,name=_('LoadDICOMAction'))[0]
        action.connect(_('triggered()'),self.onOpenBrowserWidget)
      except IndexError:
        logging.error(_('Could not connect to the main window DICOM button'))
    ...
    self.databaseRefreshRequestTimer.connect(_('timeout()'), self.requestDatabaseRefresh)
    ...
    for pluginClass in slicer.modules.dicomPlugins:
      ...
      self.checkBox.connect(_('stateChanged(int)'), self.onPluginStateChanged)
      self.checkBoxByPlugins.append(self.checkBox)

  def onPluginStateChanged(self, state):
    settings = qt.QSettings()
    settings.beginWriteArray(_('DICOM/disabledPlugins'))
    ...
    for pluginClass in slicer.modules.dicomPlugins:
      if pluginClass not in plugins:
        ...
        settings.setValue(pluginClass, _('disabled'))
        arrayIndex += 1
  ...
  def addListenerObservers(self):
    ...
    if slicer.dicomListener.process is not None:
      slicer.dicomListener.process.connect(_('stateChanged(QProcess::ProcessState)'), self.onListenerStateChanged)
  ...
  def removeListenerObservers(self):
    if not hasattr(slicer, _('dicomListener')):
      return
    if slicer.dicomListener.process is not None:
      slicer.dicomListener.process.disconnect(_('stateChanged(QProcess::ProcessState)'), self.onListenerStateChanged)
  ...
  def onListenerStateChanged(self, newState=None):
    ...
    """
    if hasattr(slicer, _('dicomListener')) and slicer.dicomListener.process is not None:
    ...
    if newState == 0:
      ...
      if hasattr(slicer.modules, _('DICOMInstance')):  # custom applications may not have the standard DICOM module
        slicer.modules.DICOMInstance.stopListener()
    if newState == 1:
      self.ui.listenerStateLabel.text = _("starting")
    if newState == 2:
      port = str(slicer.dicomListener.port) if hasattr(slicer, _('dicomListener')) else _("unknown")
  ...
  def requestDatabaseRefresh(self):
    logging.debug_(("Database refresh is requested"))
    if slicer.dicomDatabase.isOpen:
  ...
  def onToggleServer(self):
    if self.testingServer and self.testingServer.qrRunning():
      ...
      if not self.testingServer:
        ...
        self.exeDir = self.exeDir + _('/../CTK-build/DCMTK-build')
      ...
      # with standard naming conventions)
      self.dataDir =  slicer.app.slicerHome + _('/../../Slicer4/Testing/Data/Input/CTHeadAxialDicom')
      files = glob.glob(self.dataDir+_('/*.dcm'))
  ...
  def onRunListenerAtStart(self, toggled):
    settings = qt.QSettings()
    settings.setValue(_('DICOM/RunListenerAtStart'), toggled)
  ...
  def onClearDatabase(self):
    ...
    if len(patientIds) == 0:
      slicer.util.infoDisplay_(("DICOM database is already empty."))
